...
def analyze_skill_gaps() -> List[Dict[str, Any]]:
    recorded_gaps = get_skill_gaps(limit=15)
    analyzed = []
    
    # If database is fresh, supply key high-yield target tools
    detected_names = {r["skill_name"].title() for r in recorded_gaps}
    base_skills = ["Snowflake", "Dbt", "Alteryx", "Google Analytics", "Power Automate", "Bigquery"]
    ...
    for s in base_skills:
        if s not in detected_names:
            recorded_gaps.append({
                "skill_name": s,
                "frequency": 3,
                "category": SKILL_ROADMAP_DATABASE.get(s, DEFAULT_ROADMAP)["category"],
                "last_seen": "Recent"
            })
